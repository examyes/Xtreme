...
void CLexicalAnalyzer::analyze_char(char val_ch, size_t row, CTokenStream& token_stream)
{
    ...
    if (EN_LEX_IN_STRING != m_lex_status && StringUtils::whitespace_p(val_ch))
    {
        // 空白符不在字符串中，尝试解析一次
...
void CLexicalAnalyzer::analyze_char_out_string(char val_ch,
                                               size_t row,
...
ETokenType CLexicalAnalyzer::parse_token_type_from_lexeme()
{
    ...
    if (StringUtils::float_p(m_lexeme))
    {
        return TOKEN_TYPE_FLOAT;
    }

    if (StringUtils::int_p(m_lexeme))
    {
